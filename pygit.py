...
class ObjectType(enum.Enum):
    """
    对象类型枚举。还有其他类型，但我们不需要。
    请参阅 git 源代码（git/cache.h）中的 "enum object_type"。
    """
    commit = 1
    tree = 2
    blob = 3
...
def hash_object(data, obj_type, write=True):
    """
    计算给定类型对象数据的哈希值，如果 "write" 为 True，则将哈希值写入对象存储。
    返回 SHA-1 对象哈希值作为十六进制字符串。
    """
    header = '{} {}'.format(obj_type, len(data)).encode()
    full_data = header + b'\x00' + data
    sha1 = hashlib.sha1(full_data).hexdigest()
    if write:
        # gpt：Git使用哈希值的前两个字符作为目录名，其余部分作为文件名
...
def find_object(sha1_prefix):
    """
    查找具有给定 SHA-1 前缀的对象，并返回对象存储中对象的路径，
    或引发 ValueError 如果找不到对象或存在多个具有此前缀的对象。
    """
...
def read_object(sha1_prefix):
    """
    读取给定 SHA-1 前缀的对象，并返回一个包含对象类型和数据字节的元组，
    如果找不到对象，则引发 ValueError。
    """
...
def cat_file(mode, sha1_prefix):
    """
    将给定 SHA-1 前缀的对象的内容（或信息）写入 stdout。
    如果 mode 是 'commit'、'tree' 或 'blob'，则打印对象的原始数据字节。
    如果 mode 是 'size'，则打印对象的大小。
    如果 mode 是 'type'，则打印对象的类型。
    如果 mode 是 'pretty'，则打印对象的格式化版本。
    """
...
def read_index():
    ...
    try:
        ...
        return []
    # 校验 checksum（防止索引损坏）
    # data[:-20] 是索引主体部分，data[-20:] 是原始 checksum。
    digest = hashlib.sha1(data[:-20]).digest()
    assert digest == data[-20:], 'invalid index checksum'
    # 解读文件头部结构：
    # signature: 应该是 b'DIRC'，表示 Git 索引文件
    # version: 通常是 2（支持的版本）
    # num_entries: 文件中包含多少个索引条目
    ...
    # 准备开始读取每个索引项（entry），
    # 索引条目的数据位于 header（前 12 字节）之后，checksum（最后 20 字节）之前。
    entry_data = data[12:-20]
    entries = []
    i = 0
    # 一个 entry 至少 62 字节，如果 i+62 越界说明已经读完了。
    while i + 62 < len(entry_data):
        # 每个 entry 的前 62 字节字段固定格式（由 10 个整数、一个 SHA1 和一个标志位组成
        fields_end = i + 62
        fields = struct.unpack('!LLLLLLLLLL20sH', entry_data[i:fields_end])
        # 解析路径名（变长字段）（Git 索引条目中，路径名以 null 字符 \x00 结尾。）
...
def get_status():
    ...
    # 4.检查哪些是新增文件（工作区有，暂存区没有）
    new = paths - entry_paths
    # 5.检查哪些是删除文件（暂存区有，工作区没有）
...
def diff():
    ...
    # 从索引文件中读取所有的条目，构建一个字典，以 文件路径 为 key，IndexEntry 为 value。
    entries_by_path = {e.path: e for e in read_index()}
    # 遍历所有 changed 的文件，进行 diff 比较
    for i, path in enumerate(changed):
        ...
        # 使用 difflib.unified_diff 比较两者差异
...
def add(paths):
    ...
    """
    将文件添加到暂存区
    .git/index 文件是 Git 暂存区的物理实现。
    当执行 git add 时，文件内容被哈希并存储为 blob 对象，索引条目记录指向这个对象。
    """
